...
import android.bytewebkit.CacheManager.CacheResult;
...
public final class UrlInterceptRegistry {
    ...
    private static synchronized LinkedList getHandlers() {
        if(mHandlerList == null)
            mHandlerList = new LinkedList<android.bytewebkit.UrlInterceptHandler>();
        return mHandlerList;
    }
    ...
    public static synchronized boolean registerHandler(
            android.bytewebkit.UrlInterceptHandler handler) {
    ...
    public static synchronized CacheResult getSurrogate(
        ...
        while (iter.hasNext()) {
            android.bytewebkit.UrlInterceptHandler handler = (android.bytewebkit.UrlInterceptHandler) iter.next();
     ...
     */
    @Deprecated
    public static synchronized android.bytewebkit.PluginData getPluginData(
            String url, Map<String, String> headers) {
        if (urlInterceptDisabled()) {
            return null;
        }
        ...
        while (iter.hasNext()) {
            android.bytewebkit.UrlInterceptHandler handler = (android.bytewebkit.UrlInterceptHandler) iter.next();
            android.bytewebkit.PluginData data = handler.getPluginData(url, headers);
            if (data != null) {
                return data;
            }
        }
